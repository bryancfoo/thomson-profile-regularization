...
from ThomsonScatteringArbitrary.fitting import _build_grad_problem
...
def main():
    ...
    with h5py.File("ThomsonScatteringArbitrary/examples/data/data_kappa.h5", "r") as f:
    ...
    # ── sampling smoke ───────────────────────────────────────────────────────────
    print("1. SGLD posterior sampling (general path, tiny budget)")
    problem = _build_grad_problem(P, V, meas, params_settings=pars, shard_time=False)
    samp = run_sgld_posterior(
