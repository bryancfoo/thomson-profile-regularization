...
3. forward parity:   scattered_power_wavelength with every species routed
                     through the general quadrature vs the analytic
                     (tabulated) path, on EPW-like and IAW-like parameter sets
4. gradient sanity:  finite gradients of the general path w.r.t. T and kappa

Run from the repo root:  python -m ThomsonScattering.benchmarks.parity_dispersion
"""
import sys
...
from ThomsonScattering.dispersion import _Zprime
...
# ── 3. forward parity: general quadrature vs analytic path ──────────────────
# (Bit-level parity of the analytic path against the ORIGINAL package was
# validated before it was deleted — see the promotion commit. Here the
# regression target is the general quadrature path against the analytic
# tabulated-Z' path through the full forward model.)
print("\n3. Forward-model parity: general quadrature vs analytic path")
import ThomsonScattering.forward as new_fwd
...
sg_gen = resolve_distribution({"model": "super_gaussian_numeric",
                               "x_max": 12.0, "n_points": 4001})
for case_name, pe_val in [("EPW p=2", 2.0), ("EPW p=3.2", 3.2)]:
    pe_c = jnp.full((1, Nt), pe_val)
    analytic = new_fwd.scattered_power_wavelength(
    ...
    general = new_fwd.scattered_power_wavelength(
        n, ue, ui, Te, Ti, efract, ifract, ion_z, ion_a, **epw,
        e_models=(sg_gen,), i_models=(SuperGaussian(),),
        e_shapes=((pe_c[0],),), i_shapes=((pi_arr[0],),),
    )
    check(f"forward {case_name} general-vs-analytic",
          rel_err(general, analytic, floor=1e-8), 1e-3)
...
# every species routed through the general quadrature, vs the analytic path
new_gen = new_fwd.scattered_power_wavelength(
    n, ue, ui2, Te, Ti2, efract, ifract2, ion_z2, ion_a2, **iaw,
    e_models=(sg_gen,), i_models=(sg_gen, sg_gen),
    e_shapes=((pe[0],),), i_shapes=((pi2[0],), (pi2[1],)),
)
check("forward IAW D+C via general quadrature",
      rel_err(new_gen, analytic, floor=1e-8), 1e-3)
