...
EPW_LAM = np.linspace(230e-9 - 40e-9, 230e-9 + 40e-9, 1024)  # m
...
# ── Plot toggles ─────────────────────────────────────────────────────────────
PLOT_OVERVIEW  = False   # 2D pcolormesh of raw FLASH fields
PLOT_LINEOUTS  = False   # time-resolved ne, ni, Te, Ti, vx, α at XX_UM
PLOT_STREAK    = True   # synthetic Thomson EPW streak
...
def compute_thomson_streak(xx_um: float) -> np.ndarray:
    ...
    velx = get_lineout("velx", xx_um) * 1e-2 * 0   # cm/s → m/s
...
def main():
    ...
    if PLOT_LINEOUTS or PLOT_STREAK:
